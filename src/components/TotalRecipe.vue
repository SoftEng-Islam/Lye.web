...
div(class="lyeWidget flex flex-col min-h-[200px] border border-transparent hover:border-blue-500")
	...
	ul(class="bg-(--LTheme3) dark:bg-(--Theme3) mt-4 p-2 pb-0 rounded-md h-full")
		...
		li(class="flex overflow-hidden py-2 first-of-type:border-none border-t border-t-(--LTheme2) dark:border-t-(--Theme2)")
			...
			span(class="text-blue-500 ml-auto duration-200") {{parseInt(OilStore.RecipeTotal.value.weightLye.toFixed(1)) || 0}} #[span(class="text-gray-300") Gram]
	...
	div(class="flex items-center overflow-hidden pt-4")
		...
		span(class="text-blue-500 ml-auto bg-(--LTheme4) dark:bg-(--Theme4) rounded-full px-2 border border-(--LTheme3) dark:border-(--Theme3)") {{ parseInt((OilStore.RecipeTotal.value.weightWater + OilStore.RecipeTotal.value.weightLye + OilStore.RecipeTotal.value.weightOils + OilStore.headerOptions.value.fragrance.value).toFixed(0)) || 0 }} #[span(class="text-black dark:text-white ") Gram]
</template>
