...
<template lang="pug">
//- Oils list Section
div(class="lyeWidget p-2 flex flex-col gap-2 h-96 rounded-md overflow-hidden  border border-purple-400 hover:border-purple-500" id="notes")
	div(class="w-full Head p-2 rounded-md bg-linear-to-r from-purple-500 to-gray-950/40")
		span(class="text-black dark:text-white font-bold") All Oils.
	//- Search
	div(class="relative w-full p-2 flex items-center justify-center")
		div(class="z-10 w-full pl-4 h-9 flex items-center justify-center rounded-xl bg-(--LTheme3) dark:bg-(--Theme3)")
			input(v-on:focusin="show = ''" v-on:focusout="show = 'hidden'" v-model="searchForOil" class="h-7 w-4/5 outline-none text-white placeholder:focus:opacity-40 bg-transparent border-none focus:border-none" type="text" placeholder="Search...")
	...
	div(class="OilsList w-full h-[90%] overflow-hidden flex items-center justify-center")
		PerfectScrollbar(class="h-full w-full rounded-lg border-4")
			transition-group(name="list" tag="ul" class="h-full w-full py-6 px-4 pb-6")
				li(v-for="Oil in filteredOils()" :key="Oil.Name" class="p-3 pl-2 my-2 text-sm rounded-lg h-auto flex items-center gap-2 bg-(--LTheme3) dark:bg-(--Theme3) text-(--dark-fav-color) dark:text-(--light-fav-color)" @click="ShowTheInfo(Oil)" @dblclick="ClickdOnOil(Oil)")
					span(v-show="false" class="bg-(--light-fav-color) dark:bg-(--dark-fav-color) text-black font-bold px-2 rounded-md inline-block h-full")
						| {{ Oil.Id }}
					button(type="button" class="h-6 w-6 mx-1  font-bold text-xl rounded-md opacity-50 flex-none bg-(--light-fav-color) dark:bg-(--dark-fav-color) text-black hover:opacity-100" @click="ClickdOnOil(Oil)")
