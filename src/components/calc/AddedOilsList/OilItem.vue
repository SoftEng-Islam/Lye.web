...
li( class="hover:shadow-2xl duration-200 flex overflow-hidden items-center py-2 first-of-type:border-none border-t border-t-(--LTheme3) dark:border-t-(--Theme3) rounded-md p-2 bg-(--LTheme4) dark:bg-(--Theme4)")
	//- remove Oil
	button(type="button" class="mr-2 w-7 h-7 shadow cursor-pointer flex items-center justify-center rounded-sm hover:opacity-50 flex-none bg-(--LTheme3) dark:bg-(--Theme3)" @click="RemoveThisOil(props.oil)")
	...
	div(class="overflow-hidden w-1/5 flex flex-col gap-1 items-center justify-center ml-auto")
		select(class="w-full p-1 outline-none rounded-md bg-(--LTheme3) dark:bg-(--Theme3) border-r-4 border-(--LTheme3) dark:border-(--Theme3) text-green-500" :name="props.oil.Name")
			option(value="Gram") g
			option(value="Kilo") K.g
		input(class="w-full p-1 text-black dark:text-white outline-none rounded-md border bg-(--LTheme3) dark:bg-(--Theme3)" type="number"  maxlength="3" name="oilWeight" :id="props.oil.Name" v-model="weightOfOil" @input="SetOilWeight(props.oil.Name)")
</template>
