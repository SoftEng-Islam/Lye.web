...
div(class="lyeWidget hover:shadow-2xl dark:shadow-none border-yellow-800 dark:border-yellow-500")
	...
	perfect-scrollbar(class="w-full mt-4 rounded-lg bg-(--LTheme3) dark:bg-(--Theme3)")
		ul(class="p-2 pt-4 rounded-md overflow-hidden")
			...
			li(class="flex items-center justify-between overflow-hidden py-2 border-t border-t-zinc-500 first-of-type:border-t-0 last-of-type:border-b-0" v-for="(value, name, index) in OilProperties" :key="index")
				span(class="float-left text-black dark:text-white flex items-center")
					button(class="w-4 h-4 rounded-full flex items-center justify-center bg-yellow-500 text-sm mr-2 " v-tippy="{ content: aryRanges[index] }") !
					| {{ name }} #[span(class="text-black dark:text-white") :]
				span(class="text-yellow-700 dark:text-yellow-500") {{value}}
</template>
