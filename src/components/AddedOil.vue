...
li( class="hover:shadow-2xl duration-200 flex overflow-hidden items-center py-2 first-of-type:border-none border-t border-t-(--LTheme3) dark:border-t-(--Theme3) rounded-md p-2 bg-(--LTheme4) dark:bg-(--Theme4)")
	//- remove Oil
	button(type="button" class="mr-2 w-7 h-7 cursor-pointer flex items-center justify-center rounded-sm hover:opacity-50 flex-none bg-(--LTheme3) dark:bg-(--Theme3)" @click="RemoveThisOil(oil)")
	...
	//- Oil Name
	span(class="pr-2 w-3/5 text-sm text-black dark:text-white overflow-hidden overflow-ellipsis whitespace-wrap") {{ oil.Name }}
	div(class="overflow-hidden w-1/5 flex flex-col gap-1 items-center justify-center ml-auto")
		...
		input(class="w-full p-1 text-black dark:text-white outline-none rounded-md border bg-(--LTheme3) dark:bg-(--Theme3)" type="number" maxlength="3" v-model="weightOfOil" @input="SetOilWeight(oil.Name)")
</template>
