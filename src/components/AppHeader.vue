...
// Func to Apply The Selected color Theme
function setColorTheme(color: number): void {
	LiColorActive.value = color;
	switch (color) {
		case 1:
...
header(class="overflow-hidden w-full top-0 left-0 h-20 flex items-center px-4 bg-(--LTheme1) dark:bg-(--Theme1)" data-tauri-drag-region style="user-select: none;")
	...
	div(class="px-2 mr-3 h-8 overflow-hidden rounded-full flex items-center justify-center gap-x-2 bg-(--LTheme4) dark:bg-(--Theme4)")
		...
		button(type="button", @click="setColorTheme(1)" :class="['w-5 h-5 shadow-md rounded-full cursor-pointer hover:scale-90 duration-150 bg-linear-to-r from-(--L-gra) to-gray-950/70', LiColorActive == 1 ? 'scale-110 mx-1' : '']")
