...
// Template ref
// input(name = "searchForOil" ref = "lookingForOil" id = "searchForOil" v - on: focusin = "show = ''" v - on: focusout = "show = 'hidden'" v - model="searchForOil" class= "h-7 w-4/5 outline-none text-black dark:text-white font-bold placeholder:focus:opacity-40 bg-transparent border-none focus:border-none" type = "text" placeholder = "Search...")
// const lookingForOil = ref<HTMLInputElement | null>(null);
// console.log(`${lookingForOil.value?.value}`);
...
div(class="lyeWidget hover:shadow-2xl dark:shadow-none border-purple-800 dark:border-purple-500")
	...
	//- Search
	div(class="relative w-full p-2 mt-2 flex items-center justify-center")
	...
	//- Oils
	div(class="OilsList w-full h-[90%] overflow-hidden flex items-center justify-center bg-(--LTheme4) dark:bg-(--Theme4) rounded-lg py-3")
		PerfectScrollbar(class="h-full w-full mt-4" :effectData="filteredOils()")
			transition-group(enter-active-class="animate__animated animate__fadeInLeft" leave-active-class="animate__animated animate__fadeOutLeft" mode="out-in" tag="ul" class="h-full w-full py-2 px-4 pb-6")
				li(v-for="Oil in filteredOils()" :key="Oil.Name" class="p-3 pl-2 my-2 text-sm rounded-lg h-auto flex items-center gap-2 bg-(--LTheme3) dark:bg-(--Theme3) text-(--dark-fav-color) dark:text-(--light-fav-color)" @click="OilStore.showTheInfo(Oil)" @dblclick="OilStore.ClickedOil(Oil)")
					span(v-show="false" class="bg-(--light-fav-color) dark:bg-(--dark-fav-color) text-black font-bold px-2 rounded-md inline-block h-full")
						| {{ Oil.Id }}
					button(type="button" class="h-7 w-7 mx-1 font-bold text-xl cursor-pointer rounded-md opacity-50 flex-none bg-(--LTheme1) dark:bg-(--Theme1) text-black dark:text-white hover:opacity-100" @click="OilStore.ClickedOil(Oil)")
...
.OilsList
	...
	.ps__thumb-y
		width: 11px
		background-color: var(--fav-dark-color) !important
